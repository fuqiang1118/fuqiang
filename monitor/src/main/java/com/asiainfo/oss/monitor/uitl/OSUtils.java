...
public class OSUtils {
    ...
    private  Connection conn = RemoteCommandUtil.login(ip,username,password);
...
//        }
    ...
    public void monitorCPU() {
        ...
        if (cpuUsage > 10.0) {
            emailService.sendSimpleEmail("1156517308@qq.com", "服务器cpu使用率过高，请注意查看", "测试内容");
        }
        long memoryUsage = memoryUsage();
        log.info("memoryUsage:" + memoryUsage);
        if ((memoryUsage / 1024) < 100) {
            emailService.sendSimpleEmail("1156517308@qq.com", "服务器内存剩余空间不足，请注意查看", "测试内容");
        }
    }
